...
public class UsersDAO implements dao<User> {
    ...
    @Override
    public int getCount() {
        PreparedStatement stmt = conn.prepareStatement("select count(*) cnt from users");
        ResultSet resultSet = stmt.executeQuery();
        int count=0;
        if (resultSet.next()) {
    ...
    public void put(User user) {
        PreparedStatement st = conn.prepareStatement("insert into users (id, username, password, img) values (?,?,?,?);commit;");
