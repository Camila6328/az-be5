...
public class CollectionFamilyDao implements FamilyDao {
    ...
    ///saveFamily (accepts the Family type) - updates an existing family in Database if
    // if that one already exists, saves at the end of the list - if not.
    @Override
...
// the number of people equal to the number of transfered people.
    ...
    public Family bornChild(Family dfm, String gender) {
        Family fm = new Family(dfm.mother,dfm.father);
    ...
    public Family adoptChild(Family fm, Human child) {
        Family nfm = new Family(fm.mother,fm.father);
        nfm.addChild(child);
        return nfm;
    }
